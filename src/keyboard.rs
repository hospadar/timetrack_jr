use std::{sync::{Mutex}, fmt::Display, time::Instant, thread};
...
impl Display for Keypress {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if self.shift {output.push("Shift".to_string());}
        if self.ctrl {output.push("Ctrl".to_string());}
        if self.alt {output.push("Alt".to_string());}
        if self.meta {output.push("Meta".to_string());}
...
static LAST_KEYPRESS: Lazy<Mutex<Keypress>> = Lazy::new(|| {Mutex::new(Keypress { shift: false, ctrl: false, alt: false, meta: false, key: "".to_string() })});
